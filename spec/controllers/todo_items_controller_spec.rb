require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe TodoItemsController do

  # This should return the minimal set of attributes required to create a valid
  # TodoItem. As you add validations to TodoItem, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {entry: "O HAI", :todo_list_id => @todo_list.id}
  end

  # This should return the minimal set of values that should be in the session
  # in order to pass any filters (e.g. authentication) defined in
  # TodoItemsController. Be sure to keep this updated too.
  def valid_session
    {}
  end

  before do
    @todo_list = TodoList.create!({ :title  => "Sample List" })
  end

  describe "GET index" do
    it "assigns all todo_items as @todo_items" do
      todo_item = TodoItem.create! valid_attributes
      get :index, {:todo_list_id => @todo_list.id}, valid_session
      assigns(:todo_items).should eq([todo_item])
    end
  end

  describe "GET show" do
    it "assigns the requested todo_item as @todo_item" do
      todo_item = TodoItem.create! valid_attributes
      get :show, {:todo_list_id => @todo_list.id, :id => todo_item.to_param}, valid_session
      assigns(:todo_item).should eq(todo_item)
    end
  end

  describe "GET new" do
    it "assigns a new todo_item as @todo_item" do
      get :new, {:todo_list_id => @todo_list.id}, valid_session
      assigns(:todo_item).should be_a_new(TodoItem)
    end
  end

  describe "GET edit" do
    it "assigns the requested todo_item as @todo_item" do
      todo_item = TodoItem.create! valid_attributes
      get :edit, {:todo_list_id => @todo_list.id, :id => todo_item.to_param}, valid_session
      assigns(:todo_item).should eq(todo_item)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new TodoItem" do
        expect {
          post :create, {:todo_list_id => @todo_list.id, :todo_item => valid_attributes}, valid_session
        }.to change(TodoItem, :count).by(1)
      end

      it "assigns a newly created todo_item as @todo_item" do
        post :create, {:todo_list_id => @todo_list.id, :todo_item => valid_attributes}, valid_session
        assigns(:todo_item).should be_a(TodoItem)
        assigns(:todo_item).should be_persisted
      end

      it "redirects to the created todo_item" do
        post :create, {:todo_list_id => @todo_list.id, :todo_item => valid_attributes}, valid_session
        response.should redirect_to(todo_list_todo_item_url(@todo_list, TodoItem.last))
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved todo_item as @todo_item" do
        # Trigger the behavior that occurs when invalid params are submitted
        TodoItem.any_instance.stub(:save).and_return(false)
        post :create, {:todo_list_id => @todo_list.id, :todo_item => {}}, valid_session
        assigns(:todo_item).should be_a_new(TodoItem)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        TodoItem.any_instance.stub(:save).and_return(false)
        post :create, {:todo_list_id => @todo_list.id, :todo_item => {}}, valid_session
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested todo_item" do
        todo_item = TodoItem.create! valid_attributes
        # Assuming there are no other todo_items in the database, this
        # specifies that the TodoItem created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        TodoItem.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, {:todo_list_id => @todo_list.id, :id => todo_item.to_param, :todo_item => {'these' => 'params'}}, valid_session
      end

      it "assigns the requested todo_item as @todo_item" do
        todo_item = TodoItem.create! valid_attributes
        put :update, {:todo_list_id => @todo_list.id, :id => todo_item.to_param, :todo_item => valid_attributes}, valid_session
        assigns(:todo_item).should eq(todo_item)
      end

      it "redirects to the todo_item" do
        todo_item = TodoItem.create! valid_attributes
        put :update, {:todo_list_id => @todo_list.id, :id => todo_item.to_param, :todo_item => valid_attributes}, valid_session
        response.should redirect_to(todo_list_todo_item_url(@todo_list, todo_item))
      end
    end

    describe "with invalid params" do
      it "assigns the todo_item as @todo_item" do
        todo_item = TodoItem.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        TodoItem.any_instance.stub(:save).and_return(false)
        put :update, {:todo_list_id => @todo_list.id, :id => todo_item.to_param, :todo_item => {}}, valid_session
        assigns(:todo_item).should eq(todo_item)
      end

      it "re-renders the 'edit' template" do
        todo_item = TodoItem.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        TodoItem.any_instance.stub(:save).and_return(false)
        put :update, {:todo_list_id => @todo_list.id, :id => todo_item.to_param, :todo_item => {}}, valid_session
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested todo_item" do
      todo_item = TodoItem.create! valid_attributes
      expect {
        delete :destroy, {:todo_list_id => @todo_list.id, :id => todo_item.to_param}, valid_session
      }.to change(TodoItem, :count).by(-1)
    end

    it "redirects to the todo_items list" do
      todo_item = TodoItem.create! valid_attributes
      delete :destroy, {:todo_list_id => @todo_list.id, :id => todo_item.to_param}, valid_session
      response.should redirect_to(todo_list_todo_items_url(@todo_list))
    end
  end

end
